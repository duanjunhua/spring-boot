...
public class MinioUtil {
    ...
    public void createMinioClient(){
        try{
            ...
            log.error("[Minio工具类] >>>> MinIO服务异常：", e);
        }
    }
    ...
    /**
     * 启动SpringBoot 容器的时候初始化Bucket,如果没有Bucket则创建
     */
    ...
    public static Optional<Bucket> getBucket(String bucketName) throws Exception{
        return getAllBuckets().parallelStream().filter(v -> StringUtils.pathEquals(v.name(), bucketName)).findFirst();
    }
    ...
    public static boolean isFolderExist(String bucketName, String objectName){
        ...
        try{
            ...
            for (Result<Item> result : results){
                ...
                if(item.isDir() && StringUtils.pathEquals(objectName, item.objectName())){
                    exist = true;
                    break;
                }
            }
    ...
    public static List<Item> getAllObjectsByPrefix(String bucketName, String prefix, boolean recursive) throws Exception{
        ...
        Iterable<Result<Item>> objectsIterator = minioClient.listObjects(
                ListObjectsArgs.builder()
                        ...
                        .recursive(recursive)
                        .build());
        if(null == objectsIterator) return list;
     ...
     */
    public static ObjectWriteResponse copyFile(String bucketName, String objectName, String targetBucketName, String targetObjectName) throws Exception {
        return minioClient.copyObject(CopyObjectArgs.builder()
                        .source(CopySource.builder().bucket(bucketName).object(objectName).build())
                        .bucket(targetBucketName).object(targetObjectName)
                .build());
    }
